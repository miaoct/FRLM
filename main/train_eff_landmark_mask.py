...
from function import train_EFF_landmark_mask, valid_EFF_landmark_mask
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, epoch_number + 1):
        # scheduler.step()
        loss_train, loss_l1, logloss_train, acc_train, auc_train, eer_train = train_EFF_landmark_mask(
            trainLoader,
            model,
            epoch,
            epoch_number,
        ...
        if cfg.VALID_STEP != -1 and epoch % cfg.VALID_STEP == 0:
            loss_valid, logloss_valid, acc_valid, auc_valid, eer_valid = valid_EFF_landmark_mask(
