...
from function import train_XCP_landmark_mask, valid_XCP_landmark_mask
...
if __name__ == "__main__":
    ...
    for iter_num in range(max_iter+1):
        ...
        loss_batch, loss_l1, acc_batch = train_XCP_landmark_mask(
            train_iter,
            model,
            iter_num,
            max_iter,
        ...
        l1loss_dict = {"train_l1loss": loss_l1}
        acc_dict = {"train_acc": acc_batch}

        if (iter_num != 0 and (iter_num+1) % iter_per_valid == 0):
            loss_valid, logloss_valid, acc_valid, auc_valid, eer_valid = valid_XCP_landmark_mask(
            ...
            logger.info(
                "---Epoch:{:>3d} Best_logloss:{:>5.4f} Best_Acc:{:>5.4f}% Best_Auc:{:>5.4f}% Best_EER:{:>5.4f}% ---".format(
                    epoch, best_logloss, best_acc*100, best_auc*100, best_eer*100
                )
            )
        if cfg.TRAIN.TENSORBOARD.ENABLE:
            writer.add_scalars("scalar/acc", acc_dict, epoch)
            writer.add_scalars("scalar/loss", loss_dict, epoch)
            writer.add_scalars("scalar/l1_loss", l1loss_dict, epoch)
    if cfg.TRAIN.TENSORBOARD.ENABLE:
        writer.close()
    logger.info(
